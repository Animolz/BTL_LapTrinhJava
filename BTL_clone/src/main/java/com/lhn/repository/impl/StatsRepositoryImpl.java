...
public class StatsRepositoryImpl implements StatsRepository{
    ...
    public List<Like1> countLike(Date startDate, Date endDate) {
        Session s = this.sessionFactory.getObject().getCurrentSession();
        CriteriaBuilder b = s.getCriteriaBuilder();
        CriteriaQuery<Like1> q = b.createQuery(Like1.class);
        Root root = q.from(Like1.class);
        
        List<Predicate> predicates = new ArrayList<>();
        predicates.add(b.between(root.get("createdDate").as(Date.class), startDate, endDate));
        
        q.where(predicates.toArray(new Predicate[]{}));
        
        q.multiselect(root.get("userId"), b.count(root.get("userId")));
        
        q.groupBy(root.get("userId"));
